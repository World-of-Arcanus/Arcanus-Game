...
namespace Arcanus.Server
{
	public class ScriptConsole
	{
		...
		{
			interpreter.SetFunction("out", new Arcanus.Action<object>(Print));
			interpreter.SetFunction("materials", new Arcanus.Action(PrintMaterials));
			interpreter.SetFunction("materials_between", new Arcanus.Action<double, double>(PrintMaterials));
			interpreter.SetFunction("find_material", new Arcanus.Action<string>(FindMaterial));
			interpreter.SetFunction("position", new Arcanus.Action(PrintPosition));
			interpreter.SetFunction("get_position", new Arcanus.Func<Vector3i>(GetPosition));
			interpreter.SetVariable("turtle", Turtle);
			interpreter.SetFunction("set_block", new Arcanus.Action<double, double, double, double>(SetBlock));
			interpreter.SetFunction("get_block", new Arcanus.Func<double, double, double, int>(GetBlock));
			interpreter.SetFunction("get_height", new Arcanus.Func<double, double, double>(GetHeight));
			interpreter.SetFunction("get_mapsize", new Arcanus.Func<int[]>(GetMapSize));
			interpreter.SetFunction("set_chunk", new Arcanus.Action<double, double, double, ushort[]>(SetChunk));
			interpreter.SetFunction("set_chunks", new Arcanus.Action<Dictionary<Xyz, ushort[]>>(SetChunks));
			interpreter.SetFunction("set_chunks_offset", new Arcanus.Action<double, double, double, Dictionary<Xyz, ushort[]>>(SetChunks));
			interpreter.SetFunction("get_chunk", new Arcanus.Func<double, double, double, ushort[]>(GetChunk));
			interpreter.SetFunction("get_chunks_from_database", new Arcanus.Func<double, double, double, double, double, double, string, Dictionary<Xyz, ushort[]>>(GetChunksFromDatabase));
			interpreter.SetFunction("copy_chunks_to_database", new Arcanus.Action<double, double, double, double, double, double, string>(CopyChunksToDatabase));
			interpreter.SetFunction("delete_chunk", new Arcanus.Action<double, double, double>(DeleteChunk));
			interpreter.SetFunction("delete_chunk_range", new Arcanus.Action<double, double, double, double, double, double>(DeleteChunkRange));
			interpreter.SetFunction("backup_database", new Arcanus.Action<string>(BackupDatabase));
			interpreter.SetFunction("clear", new Arcanus.Action(Clear));
		}
