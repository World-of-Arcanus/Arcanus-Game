...
namespace Arcanus.Server
{
	public partial class Server
	{
		...
		public bool ChangeGroup(int sourceClientId, string target, string newGroupName)
		{
			...
			Arcanus.Group newGroup = serverClient.Groups.Find(
											 delegate (Arcanus.Group grp)
				{
			...
			Arcanus.Client clientConfig = serverClient.Clients.Find(
												  delegate (Arcanus.Client client)
				{
			...
			if (targetClient != null)
			{
				...
				{
					clientConfig = new Arcanus.Client();
		...
		public bool ChangeGroupOffline(int sourceClientId, string target, string newGroupName)
		{
			...
			else
			{
				...
				Arcanus.Group oldGroup = serverClient.Groups.Find(
												 delegate (Arcanus.Group grp)
					{
		...
		public bool RemoveClientFromConfig(int sourceClientId, string target)
		{
			...
			// Get related client from config file
			Arcanus.Client targetClient = serverClient.Clients.Find(
												  delegate (Arcanus.Client client)
				{
					return client.Name.Equals(target, StringComparison.InvariantCultureIgnoreCase);
				}
			...
			{
				// Get target's group.
				Arcanus.Group targetGroup = serverClient.Groups.Find(
													delegate (Arcanus.Group grp)
					{
		...
		public bool List(int sourceClientId, string type)
		{
			switch (type)
			{
				...
				case "-g":
					...
					foreach (Arcanus.Group currenGroup in serverClient.Groups)
					{
				...
				case "-sc":
					...
					foreach (Arcanus.Client currenClient in serverClient.Clients)
					{
		...
		public bool SetSpawnPosition(int sourceClientId, string targetType, string target, int x, int y, int? z)
		{
			...
			switch (targetType)
			{
				case "-default":
				case "-d":
					serverClient.DefaultSpawn = new Arcanus.Spawn()
					{
						x = x,
						y = y,
						z = z
					...
					foreach (var k in clients)
					{
						...
						{
							foreach (Arcanus.Client client in serverClient.Clients)
							{
				...
				case "-g":
					...
					targetGroup.Spawn = new Arcanus.Spawn()
					{
						x = x,
						y = y,
						z = z,
				...
				case "-p":
					...
					Arcanus.Client clientEntry = serverClient.Clients.Find(
														 delegate (Arcanus.Client client)
						{
					...
					clientEntry.Spawn = new Arcanus.Spawn()
					{
						x = x,
						y = y,
						z = z,
		...
		public bool SetSpawnPosition(int sourceClientId, int x, int y, int? z)
		{
			...
			if (clientEntry == null)
			{
				clientEntry = new Arcanus.Client();
