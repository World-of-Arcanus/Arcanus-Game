...
namespace Arcanus.Server
{
	...
	public class ServerSystemLoadServerClient : ServerSystem
	{
		...
		public void LoadServerClient(Server server)
		{
			...
			else
			{
				try
				{
					...
					{
						XmlSerializer deserializer = new XmlSerializer(typeof(ServerClient), typeof(ServerClient).GetNestedTypes());
				...
				catch //This if for the original format
				{
					using (Stream s = new MemoryStream(File.ReadAllBytes(Path.Combine(GameStorePath.gamepathconfig, filename))))
					{
						...
						d.Load(sr);
						server.serverClient.Format = int.Parse(XmlTool.XmlVal(d, "/ArcanusServerClient/Format"));
						server.serverClient.DefaultGroupGuests = XmlTool.XmlVal(d, "/ArcanusServerClient/DefaultGroupGuests");
						server.serverClient.DefaultGroupRegistered = XmlTool.XmlVal(d, "/ArcanusServerClient/DefaultGroupRegistered");
					}
		...
		public void SaveServerClient(Server server)
		{
			...
			XmlSerializer serializer = new XmlSerializer(typeof(ServerClient), typeof(ServerClient).GetNestedTypes());
